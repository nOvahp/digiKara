...
const TransactionCard: React.FC<TransactionCardProps> = ({ transaction }) => {
    ...
    return (
        <div className={`w-full bg-white border-2 ${borderColor} rounded-xl p-3 flex items-center justify-between shadow-sm`}>
            {/* Icon Box */}
            <div className={`${iconBgColor} w-12 h-12 rounded-lg flex items-center justify-center shrink-0`}>
                ...
                <div className="w-full flex justify-between items-center mt-1 border-t border-dashed border-gray-200 pt-1">
                    {isWithdrawal ? (
                         ...
                         <>
                            ...
                            <div className="flex items-center gap-1 text-[10px] text-[#818898] font-['PeydaWeb'] font-semibold">
                                <span>تاریخ:</span>
                                <span className="font-num-medium font-semibold text-[#0D0D12]">{toFarsiNumber(transaction.date)}</span>
                            </div>
                         </>
                    ) : (
                        // Deposit Details
                        <>
                            ...
                            <div className="flex items-center gap-1">
                                <span className="text-[10px] text-[#818898] font-['PeydaWeb'] font-semibold">{transaction.codeLabel}:</span>
                                <span className="text-[10px] text-[#818898] font-['Poppins'] font-medium">{transaction.code}</span>
                            </div>
                        </>
                    )}
                </div>
