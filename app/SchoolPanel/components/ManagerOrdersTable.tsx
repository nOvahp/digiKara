...
const ManagerOrdersTable = () => {
    ...
    const filteredOrders = orders.filter(order => {
        ...
        const matchesStatus = 
            ...
            filterStatus === 'pending' ? !isCompleted : true;
                    ...
                    <div className="flex items-center gap-2" ref={filterRef}>
                        <div className="relative">
                                ...
                                <span className="text-xs font-semibold font-['PeydaWeb'] text-[#666D80]">
                                    {filterStatus === 'all' ? 'همه' : filterStatus === 'completed' ? 'تکمیل شده' : 'در انتظار'}
                                </span>
                            </div>
                ...
                <div className="w-full overflow-x-auto">
                    <table className="w-full min-w-[800px]">
                        ...
                        <tbody className="divide-y divide-[#F2F4F7]">
                            {loading ? (
                                <tr>
                                    <td colSpan={8} className="text-center py-8 text-gray-500">در حال بارگذاری...</td>
                                </tr>
                            ) : currentOrders.length === 0 ? (
                                <tr>
                                    <td colSpan={8} className="text-center py-8 text-gray-500">سفارشی یافت نشد</td>
                                </tr>
                            ) : (
                                    ...
                                    <tr key={order.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => setSelectedOrder(order)}>
                                        ...
                                        <td className="px-6 py-4">
                                            <div className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border
                                                ${order.status === 'sent' || order.status === 'تکمیل شده'
