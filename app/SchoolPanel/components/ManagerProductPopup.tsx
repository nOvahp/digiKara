...
const ManagerProductPopup = ({ onClose, product: initialProduct, onApprove }: ManagerProductPopupProps) => {
                ...
                <div className="self-stretch flex-col justify-start items-end gap-4 flex w-full flex-1 overflow-y-auto no-scrollbar pb-20">
                    ...
                    <div className="w-full flex justify-between items-center bg-[#F9FAFB] p-3 rounded-xl border border-[#DFE1E7]">
                         ...
                         {(model_data.prices && model_data.prices.length > 0) && (
                            <span className="text-xs text-amber-600 bg-amber-50 px-2 py-1 rounded-lg">چند قیمتی</span>
                         )}
                    </div>
                    ...
                    <div className="self-stretch flex-col justify-start items-start gap-2 flex w-full">
                        ...
                        <div className="self-stretch h-[52px] px-3 py-2 bg-white rounded-xl border border-[#DFE1E7] flex justify-between items-center">
                            ...
                            <div className={`h-5 px-2 py-0.5 rounded-2xl flex justify-start items-center ${
                                ...
                                <div className={`flex items-center gap-1 text-center text-xs font-normal font-['PeydaFaNum'] leading-[18px] tracking-wide ${
                                    product.approved
                                     ? "text-[#267666]" 
                                    : "text-[#B98900]"
                                }`}>
                                    {product.approved ? <CheckCircle className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
                                    <span className="font-num-medium">{product.approved ? "تایید شده" : "در انتظار تایید"}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    {/* Features Read Only */}
                     {model_data.features && (Object.values(model_data.features).some((arr: any) => arr.length > 0)) && (
                         <div className="flex flex-col gap-3 border-t border-[#DFE1E7] pt-4 w-full" dir="rtl">
                            ...
                            {Object.entries({
                                ...
                                <div key={label} className="flex flex-col gap-2 bg-gray-50 p-3 rounded-lg border border-[#DFE1E7]">
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-5 bg-white border-t border-gray-100/50 rounded-b-xl z-30">
                        <button
                            ...
                            className={`w-full h-12 px-6 py-3 rounded-xl flex justify-center items-center gap-2.5 transition-colors ${
                                isApproving ? "bg-gray-300 cursor-not-allowed" : "bg-[#0A33FF] cursor-pointer hover:bg-blue-700"
                            }`}
                        >
