    ...
    <div className="flex flex-1 flex-col items-center justify-start bg-background rounded-t-3xl z-10 p-6 -mt-10 pt-8 gap-6 animate-in slide-in-from-bottom-10 fade-in duration-500">
      {stage === 'WAITING' ? (
        <div className="w-full flex flex-col items-center justify-center gap-4 py-8">
          <Loader2 className="h-8 w-8 animate-spin text-[#393E46]" />
          <p className="text-[#393E46] text-lg font-semibold text-center">
            در حال ارسال کد تایید...
          </p>
          <p className="text-[#6C7278] text-sm text-center">لطفا منتظر بمانید</p>
        </div>
      ) : (
        <form onSubmit={onSubmit} className="w-full space-y-6 mt-4">
          <div className="space-y-2">
            ...
            {error && <p className="text-red-500 text-sm text-right px-1">{error}</p>}
          </div>
