...
export function LoginViewReport({ onNext, onLoginAgain }: LoginViewProps) {
  ...
  const handleLoginAgain = () => {
     if (onLoginAgain) {
        ...
        router.push("/login"); // or whatever logic for re-login
     }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await authService.changeStudentInfo(formData);
  ...
  const handleBack = () => {
      // If we are just viewing, we might want to go back to LoginView5
      // But typically "back" means cancel edit.
      if (onLoginAgain) onLoginAgain(); // Or separate onBack prop
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-black/5 p-6 sm:p-8 animate-in slide-in-from-bottom-5 fade-in duration-500 relative pt-8">
            ...
            {showSuccess ? (
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <div className="flex gap-4">
                         <div className="flex-1 space-y-2">
                            <Label className="text-right block text-xs font-bold text-[#6C7278]">پایه</Label>
                            <Input name="grade" value={formData.grade} onChange={handleChange} className="bg-[#F3F6FC] border-0 rounded-xl h-12 text-right" />
                        </div>
                        <div className="flex-1 space-y-2">
                            <Label className="text-right block text-xs font-bold text-[#6C7278]">رشته</Label>
                            <Input name="field" value={formData.field} onChange={handleChange} className="bg-[#F3F6FC] border-0 rounded-xl h-12 text-right" />
                        </div>
                    </div>
