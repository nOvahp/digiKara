...
export function LoginView7({ onNext, onBack }: LoginViewProps) {
  ...
  const isStep1Valid =
    ...
    (formState.productionExperience === 'نداشته ام' || formState.productionField.length > 0);
  const isStep2Valid =
    formState.salesExperience &&
    (formState.salesExperience === 'نداشته ام' || formState.salesField.length > 0);
              ...
              <div
                className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${
                  isSelected
                    ...
                    : 'border-[#D1D5DB] bg-white group-hover:border-gray-400'
                }`}
              >
        ...
        {fieldOptions.map((field) => {
          ...
          return (
            ...
            >
              {/* Text (Right) */}
              <span
                className={`text-sm text-right flex-1 ${isSelected ? 'text-[#393E46] font-bold' : 'text-[#6C7278]'}`}
              >
                {field}
              </span>
              ...
              {/* Checkbox (Left) */}
              <div
                className={`w-5 h-5 ml-3 rounded-md border-2 flex items-center justify-center transition-all ${
