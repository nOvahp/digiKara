...
export const authService = {
  ...
  requestOtp: async (phone: string, role: 'student' | 'manager' | 'customer' = 'student'): Promise<{ success: boolean; message?: string; status?: number }> => {
    ...
    const endpoint = role === 'manager' ? '/manager/otp' : (role === 'customer' ? '/customers/otp' : '/students/otp');
  ...
  verifyOtp: async (phone: string, code: string, role: 'student' | 'manager' | 'customer' = 'student'): Promise<{ success: boolean; token?: string; user?: UserData; message?: string }> => {
    if (USE_MOCK_BACKEND) {
        try {
            const { mockAuthService } = await import('../mockAuthService');
    ...
    const endpoint = role === 'manager' ? '/manager/otp/check' : (role === 'customer' ? '/customers/otp/check' : '/students/otp/check');
  ...
  loginCustomer: async (data: { phone: string; password: string }): Promise<{ success: boolean; token?: string; message?: string }> => {
      try {
          const payload = {
              phone: data.phone,
