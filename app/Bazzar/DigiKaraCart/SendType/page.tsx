...
export default function SendTypePage() {
  ...
  const renderEstimate = (text: string) => {
    ...
    return (
      <span className="text-[#707F81] text-xs font-['PeydaWeb'] font-light leading-5">{text}</span>
    );
  };
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="shrink-0">
                    {selectedId === item.id ? (
                      ...
                      <Circle
                        className="w-6 h-6 text-[#0C1415] opacity-20 group-hover:opacity-100 transition-opacity"
                        strokeWidth={1.5}
                      />
                    )}
                  </div>
