...
const CITIES = ["تهران", "مشهد", "اصفهان", "شیراز", "تبریز", "کرج", "اهواز", "قم", "رشت", "کرمانشاه"];

const ProductSection = ({ title, icon: Icon, items, color = "#E9B443" }: { title: string, icon: React.ElementType, items: Product[], color?: string }) => {
           ...
           {items.map((product) => (
               ...
               >
                   ...
                   <div className="flex flex-col gap-1">
                       <h3 className="font-bold text-gray-800 text-sm line-clamp-2 min-h-[40px]">{product.title}</h3>
                       <div className="flex items-center justify-between">
                          <div className="flex flex-col items-start">
                              {product.isSpecialSale && (
                                  <span className="text-[10px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded-full font-bold mb-1">فروش ویژه</span>
                              )}
...
export default function SearchStartPage() {
  ...
  const handleSearch = (query: string = searchQuery) => {
    if (query.trim()) {
      router.push(`/Bazzar/Search?q=${encodeURIComponent(query)}&location=${encodeURIComponent(location)}`);
    }
  };
    ...
    <div className="min-h-screen bg-white flex flex-col overflow-hidden animate-in fade-in duration-200">
      ...
      <div className="w-full bg-white shadow-sm p-4 z-10 sticky top-0">
          <div className="max-w-3xl mx-auto flex flex-col gap-3" dir="rtl">
             ...
             <div className="flex items-center justify-between px-1">
                 ...
                 <button onClick={handleClose} className="p-2 -ml-2 hover:bg-gray-100 rounded-full transition-colors shrink-0">
      ...
      <div className="flex-1 overflow-y-auto w-full" dir="rtl">
           <div className="max-w-3xl mx-auto p-4 sm:p-6 pb-20">
               ...
               <ProductSection title="فروش ویژه" icon={Zap} items={specialSaleProducts} color="#EF4444" />
               
               <ProductSection title="جدیدترین‌ها" icon={Sparkles} items={newCollectionProducts} color="#8B5CF6" />
               
               <ProductSection title="پرفروش‌ترین‌ها" icon={Star} items={bestSellerProducts} color="#F59E0B" />
               
               <ProductSection title="پیشنهاد ما" icon={ThumbsUp} items={suggestedProducts} color="#10B981" />
