    ...
    const getFilteredData = () => {
        ...
        const mappedData = filtered.map(order => {
            ...
            activeColumns.forEach(col => {
                switch(col) {
                    case 'Order ID': row.push(order.id); break;
                    case 'Customer': row.push(order.customer); break;
                    case 'Date': row.push(order.date); break;
                    case 'Status': row.push(order.statusText); break;
                    case 'Payment Method': row.push(order.paymentMethod); break;
                    case 'Amount': row.push(order.amount); break;
                }
            });
            return row;
        });
