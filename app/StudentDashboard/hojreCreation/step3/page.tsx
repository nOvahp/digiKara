...
export default function ShopIdentityPage() {
    ...
    return (
        <div className="w-full min-h-screen bg-white flex flex-col items-center font-sans pb-24">
            ...
            {/* Form Content */}
            <form onSubmit={handleSubmit(onSubmit)} className="flex-1 w-full max-w-md px-4 mt-6 flex flex-col gap-6 overflow-y-auto">
                
                {/* Logo Upload */}
                     <label htmlFor="logo-upload" className="w-full cursor-pointer flex justify-center">
                        {logoPreview ? (
                            <div style={{width: '100%', flexDirection: 'column', justifyContent: 'flex-start', alignItems: 'center', gap: 20.19, display: 'inline-flex'}}>
                                <div className="relative">
                                    <img style={{width: 126.21, height: 126.21, borderRadius: 63.11}} src={logoPreview} alt="Shop Logo" className="object-cover" />
                                    {/* Edit Icon Overlay */}
                                    <div className="absolute bottom-0 right-0" style={{transform: 'translate(-10%, -10%)'}}>
                                         <div style={{width: 45.44, height: 45.44, background: '#3C5A5D', borderRadius: '50%', border: '2.52px white solid', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                                            <div style={{width: 22.72, height: 22.72, position: 'relative'}}>
                                                <div style={{width: 15.14, height: 15.71, left: 3.78, top: 2.05, position: 'absolute', border: '1.56px white solid', borderRadius: '2px'}} />
                                                <div style={{width: 5.16, height: 4.87, left: 11.26, top: 4.78, position: 'absolute', border: '1.56px white solid', borderRadius: '50%'}} />
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Input 
                        ...
                        dir="rtl"
                    />
                    {errors.shopName && <span className="text-red-500 text-xs text-right font-medium">{errors.shopName.message}</span>}
                </div>
                ...
                <div className="flex flex-col gap-2">
                    ...
                    <Textarea 
                        ...
                        dir="rtl"
                    />
                     {errors.shopDescription && <span className="text-red-500 text-xs text-right font-medium">{errors.shopDescription.message}</span>}
                </div>
