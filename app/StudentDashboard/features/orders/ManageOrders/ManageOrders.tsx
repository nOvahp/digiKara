...
export default function ManageOrders() {
  ...
  return (
    <div className="w-full min-h-screen bg-white flex flex-col relative" dir="ltr">
      ...
      <div className="w-full flex flex-col pb-[150px] px-0 pt-0 gap-6" dir="rtl">
          ...
          <div className="w-full flex justify-between items-center gap-1">
            ...
            <div className="flex-1 h-9 bg-white rounded-lg border border-[#DFE1E7] flex items-center">
              <div className="px-3 py-1.5 flex items-center justify-center border-l border-[#DFE1E7]">
                <span className="text-[#737373] text-opacity-25 text-sm font-['PeydaFaNum'] font-medium">
                  {toFarsiNumber(filteredOrders.length)} نتیحه
                </span>
              </div>
          ...
          <div className="w-full flex items-center justify-start gap-2 overflow-x-auto no-scrollbar pb-1">
            {tabs.map((tab) => {
              ...
              return (
                ...
                >
                  <span
                    className={`text-sm font-['PeydaWeb'] font-semibold ${isActive ? 'text-[#393E46]' : 'text-[#737373]'}`}
                  >
                    {tab.label}
                  </span>
                </div>
        ...
        <div className="w-full bg-white rounded-xl border border-[#DFE1E7] shadow-[0px_2px_4px_-1px_rgba(13,13,18,0.06)] overflow-hidden flex flex-col">
          ...
          <div className="w-full h-16 px-5 py-2 border-b border-[#DFE1E7] flex justify-between items-center bg-white">
            <div className="text-[#0D0D12] text-base font-semibold tracking-wide">جدول سفارشات</div>
      ...
      <div
        className={`fixed bottom-0 left-0 right-0 w-full bg-white border-t border-[#DFE1E7] p-4 z-[100] transition-all duration-300 ease-in-out transform ${selectedOrderIds.length > 0 ? 'translate-y-0 opacity-100 shadow-[0_-4px_20px_rgba(0,0,0,0.08)]' : 'translate-y-full opacity-0 pointer-events-none shadow-none'}`}
      >
      ...
      {/* Product Popup */}
      {selectedOrder && (
        <PopUpProduct order={selectedOrder} onClose={() => setSelectedOrder(null)} />
      )}
    </div>
  );
}
