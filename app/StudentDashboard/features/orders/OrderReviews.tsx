...
export function OrderReviews() {
  ...
  const handleBulkDeliver = async () => {
    ...
    try {
      await Promise.all(
        selectedOrderIds.map((id) => ordersService.updateOrderStatus(id, 'تحویل به مدرسه ')),
      );
  ...
  return (
    <div
      className="w-full flex-col justify-start items-end gap-6 inline-flex mb-8 relative"
      dir="rtl"
    >
      {selectedOrder && (
