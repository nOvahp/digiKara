...
function ActionMenu({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div className="relative" ref={menuRef}>
      ...
      {isOpen && (
        <div
          className="absolute top-full left-0 mt-1 w-32 bg-white rounded-lg shadow-lg border border-[#EFF0F2] flex flex-col overflow-hidden z-20 animate-in fade-in zoom-in-95 duration-100 origin-top-left"
          dir="rtl"
        >
          <div
            onClick={(e) => {
...
export function ProductTable({ products, loading = false, onDelete }: ProductTableProps) {
  ...
  const filteredProducts = products.filter((p) => {
    const matchesSearch =
      (p.name && p.name.includes(searchQuery)) || (p.code && p.code.includes(searchQuery));
    const matchesStatus =
      statusFilter === 'all' ? true : statusFilter === 'approved' ? p.approved : !p.approved;
          ...
          <table className="w-full text-sm text-right">
            <thead className="bg-[#F9FAFB] border-b border-[#E5E7EB]">
              <tr>
                ...
                <th className="px-6 py-4 text-[#666D80] font-medium text-sm font-['PeydaWeb'] min-w-[300px]">
                  محصول
                </th>
