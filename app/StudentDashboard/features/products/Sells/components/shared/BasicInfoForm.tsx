...
interface BasicInfoFormProps {
    defaultValues?: any;
    values?: { name: string; description: string; id: string; images?: string[]; image?: string | null; imageFile?: File | null };
    onChange?: (updates: Partial<{ name: string; description: string; id: string; images?: string[]; image?: string | null; imageFile?: File | null }>) => void;
}
    ...
    return (
        <div className="w-full bg-white rounded-xl border border-[#DFE1E7] p-5 flex flex-col gap-5 shadow-[0px_1px_2px_rgba(13,13,18,0.06)]">
            ...
            <div className="flex flex-col gap-2">
                 ...
                 <div className="h-[52px] px-3 bg-white rounded-xl border border-[#DFE1E7] flex items-center">
                     <div className="w-full text-left text-[#0D0D12] text-base font-num-medium dir-ltr" dir="ltr">{val('id') || "---"}</div>
                 </div>
            </div>
            ...
            <div className="flex flex-col gap-2">
                 ...
                 <div className="w-full h-[180px] rounded-xl border border-[#DFE1E7] overflow-hidden relative group">
                    {productImages.length === 0 ? (
                         <div className="w-full h-full flex flex-col items-center justify-center bg-gray-50 text-gray-400 gap-2 cursor-pointer hover:bg-gray-100 transition-colors" onClick={() => fileInputRef.current?.click()}>
                                ...
                                <div className="absolute flex gap-2 z-10 bottom-3 right-3">
                                    <div className="flex gap-2 items-center flex-row-reverse">
                                        {productImages.map((_, i) => {
                                            ...
                                            >
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm font-num-medium transition-colors ${activeImage === num ? 'bg-[#FFD369] text-[#0D0D12]' : 'bg-[#0D0D12]/45 text-white'}`}>
                                                    {toFarsiNumber(num)}
                                                </div>
                                            </div>
                                        )})}
